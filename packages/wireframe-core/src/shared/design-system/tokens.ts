...
export function createDesignTokenGetter(override?: Override) {
  ...
  return function getToken<T extends Primitive = Primitive>(key: TokenKey, fallback?: T): T {
    ...
    if (!group) {
      const value = merged[key as keyof DesignTokens];
      return (value as unknown as T) ?? fallback ?? ("" as T);
    }

    const groupValue = merged[group as keyof DesignTokens] as Record<string, unknown> | undefined;
    ...
    if (
      ...
      return (groupValue[token] as unknown as T) ?? fallback ?? ("" as T);
    }
